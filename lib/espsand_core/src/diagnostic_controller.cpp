...
void DiagnosticController::initialize(std::uint32_t now_ms) { mode_started_ms_ = now_ms; }
...
void DiagnosticController::handle_button(io::ButtonEvent event, std::uint32_t now_ms) {
  ...
  if (event == io::ButtonEvent::kLongPress) {
    const auto next = (static_cast<std::uint8_t>(mode_) + 1U) %
                      static_cast<std::uint8_t>(DiagnosticMode::kCount);
    mode_ = static_cast<DiagnosticMode>(next);
    mode_started_ms_ = now_ms;
  }
}

DiagnosticMode DiagnosticController::mode() const { return mode_; }
